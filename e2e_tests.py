        ...
        "items": [
            {
                "type": "OFFER",
                "name": "jPhone 13",
                "id": "b5bb1a7a-1304-42ae-943b-179184cbbbb5",
...
def test_import():
    for index, batch in enumerate(IMPORT_BATCHES):
        for is_repeat in [False, True]:
            print(f"Importing batch {index}" + (" repeat" if is_repeat else ""))
            status, _ = request("/nodes", method="POST", data=batch)

            assert status == 200, f"Expected HTTP status code 200, got {status}"
        
        # input(f'Batch {index} imported. Press any key...')
...
def test_sales():
    ...
    status, response = request(f"/nodes/sales?{params}", json_response=True)
    assert status == 200, f"Expected HTTP status code 200, got {status}"
...
def test_stats():
    ...
    status, response = request(
        f"/nodes/{ROOT_ID}/statistic?{params}", json_response=True)
...
def test_delete():
    status, _ = request(f"/nodes/{ROOT_ID}", method="DELETE")
    assert status == 200, f"Expected HTTP status code 200, got {status}"
